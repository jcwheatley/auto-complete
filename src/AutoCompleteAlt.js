...
import './AutoCompleteAlt.css';

class AutoCompleteAlt extends Component {
    constructor() {
        super();
        this.state = { 
            inputValue: '',
    ...
    isActive = (i) => {
        return (this.state.activeIndex === i ? 'active-index-alt' : null);
    }

    isSelected = (i) => {
        return (this.state.selectedIndex === i ? 'selected-index-alt' : null);
    }
            ...
            <div className={"auto-complete-container-alt"}>
                ...
                <div className="results-container-alt">
                    {this.state.myArray.map((item, i) => (
                        ...
                        <div key={i} className={`results-alt ${this.isActive(i)} ${this.isSelected(i)}`}>
                            <b className={"item-name-alt"}>{item.name}</b>
                            {item.description}
                            {item.tags.map((tag, tagIndex) => {
                                if(tagIndex > 0){
                                    return (<span className={"item-tag-alt"}>, {tag}</span>);
                                }
                                else{
                                    return (<span className={"item-tag-alt"}>{tag}</span>);
                                }
